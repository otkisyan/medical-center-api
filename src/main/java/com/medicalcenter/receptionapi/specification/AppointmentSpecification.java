  ...
  public static Specification<Appointment> withPatient(String fullName) {
    return (root, query, criteriaBuilder) -> {
      ...
      if (parts.length >= 1 && !parts[0].isEmpty()) {
        predicate =
            criteriaBuilder.and(
                predicate, criteriaBuilder.like(patient.get("surname"), parts[0] + "%"));
      }
      if (parts.length >= 2 && !parts[1].isEmpty()) {
        predicate =
            criteriaBuilder.and(
                predicate, criteriaBuilder.like(patient.get("name"), parts[1] + "%"));
      }
      if (parts.length >= 3 && !parts[2].isEmpty()) {
        predicate =
            criteriaBuilder.and(
                predicate, criteriaBuilder.like(patient.get("middleName"), parts[2] + "%"));
      }
      return predicate;
    };
  }
  ...
  public static Specification<Appointment> withDoctor(String fullName) {
    return (root, query, criteriaBuilder) -> {
      ...
      if (parts.length >= 1 && !parts[0].isEmpty()) {
        predicate =
            criteriaBuilder.and(
                predicate, criteriaBuilder.like(doctor.get("surname"), parts[0] + "%"));
      }
      if (parts.length >= 2 && !parts[1].isEmpty()) {
        predicate =
            criteriaBuilder.and(
                predicate, criteriaBuilder.like(doctor.get("name"), parts[1] + "%"));
      }
      if (parts.length >= 3 && !parts[2].isEmpty()) {
        predicate =
            criteriaBuilder.and(
                predicate, criteriaBuilder.like(doctor.get("middleName"), parts[2] + "%"));
      }
      return predicate;
    };
  }
