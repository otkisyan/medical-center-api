...
public class ReceptionistController {
    ...
    public ResponseEntity<ReceptionistResponseDto> findReceptionistById(@PathVariable("id") Long id) {
        ...
        return ResponseEntity.ok()
                .body(receptionistResponseDto);
    }

    @PostMapping()
    public @ResponseBody ResponseEntity<ReceptionistResponseWithUserCredentialsDto> saveReceptionist(@RequestBody @Valid ReceptionistRequestDto receptionistRequestDto) {
        ReceptionistResponseWithUserCredentialsDto receptionistResponseWithUserCredentialsDto = receptionistService.saveReceptionist(receptionistRequestDto);
        URI location = ServletUriComponentsBuilder
                .fromCurrentRequest()
                .path("/{id}")
                .buildAndExpand(receptionistResponseWithUserCredentialsDto.getReceptionistResponseDto().getId())
                .toUri();
        return ResponseEntity.created(location)
                .body(receptionistResponseWithUserCredentialsDto);
    }
    ...
    public ResponseEntity<ReceptionistResponseDto> updateReceptionist(
        ...
        ReceptionistResponseDto receptionistResponseDto = receptionistService.updateReceptionist(receptionistRequestDto, id);
        return ResponseEntity.ok()
                .body(receptionistResponseDto);
    }
