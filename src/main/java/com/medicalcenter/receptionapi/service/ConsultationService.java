...
public class ConsultationService {
    ...
    public ConsultationResponseDto updateConsultation(Long appointmentId,
                                                      ConsultationRequestDto consultationRequestDto){
        CustomUserDetails customUserDetails = userService.getCustomUserDetails();
        Consultation consultationToUpdate = consultationRepository.findById(appointmentId).orElseThrow(ResourceNotFoundException::new);
        if(!Objects.equals(consultationToUpdate.getAppointment().getDoctor().getId(), customUserDetails.getId())){
            throw new AccessDeniedException("Only the doctor to whom the appointment belongs can change the consultation data.");
        }
        Consultation consultationUpdateRequest = ConsultationRequestDto.toEntity(consultationRequestDto);
